...
const GC = {
  ...
  difficulty: [
    ...
    // Hurdles 6–10 (Progress 26–50%, 6–10% Discount): ⚡ EXTREME NORMAL
    { minPct: 26,  speed: 24.0, gapMs: [1100, 1300], railH: 1.55, name: '⚡ EXTREME MODE', color: '#e8a838' },
    // Hurdles 11–15 (Progress 51–75%, 11–15% Discount): 🔥 EXTREME HARD
    { minPct: 51,  speed: 29.0, gapMs: [850, 1000], railH: 1.62, name: '🔥 EXTREME HARD', color: '#ff9933' },
    // Hurdles 16–20 (Progress 76–100%, 16–20% Discount): ☠️ ULTRA EXTREME
    { minPct: 76,  speed: 34.0, gapMs: [700, 820], railH: 1.70, name: '☠️ ULTRA EXTREME', color: '#ff4d4d' },
  ],
};
...
class Game {
  ...
  spawnHurdle() {
    ...
    let railH = 1.05;
    const isExtreme = this.progress > 25;
    if (hType === 0) {
      // Small/Low hurdle
      railH = (isExtreme ? 0.90 : 0.82) + Math.random() * 0.06;
    } else if (hType === 1) {
      // Medium hurdle
      railH = (isExtreme ? 1.18 : 1.08) + Math.random() * 0.08;
    } else {
      // Tall hurdle
      railH = (isExtreme ? 1.40 : 1.32) + Math.random() * 0.10;
    }
