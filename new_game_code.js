...
class Game {
  ...
  spawnHurdle() {
    ...
    if (hType === 0) {
      // Small/Low hurdle
      railH = (isExtreme ? 1.20 : 0.92) + Math.random() * 0.08;
    } else if (hType === 1) {
      // Medium hurdle
      railH = (isExtreme ? 1.50 : 1.18) + Math.random() * 0.10;
    } else {
      // Tall hurdle - requires peak jump timing to clear!
      railH = (isExtreme ? 1.78 : 1.44) + Math.random() * 0.12;
    }
...
(async function init() {
  ...
  if (regForm) {
    regForm.addEventListener('submit', async (e) => {
      ...
      if (btnRegister) {
        btnRegister.textContent = '✅ VERIFIED!';
      }
